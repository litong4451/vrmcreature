...
- **多模型支持**：`.minecraft/versions/<版本名>/vrmcreature/vrm/` 目录下可放置多个模型文件，生成时可一次**多选**多个模型
...
模型直接放在游戏目录的**版本目录**下，模组从 `.minecraft/versions/<版本名>/vrmcreature/vrm/` 读取（VRM / GLB 格式）。**支持一次放置多个模型**，每个模型一个文件，文件名使用英文即可（例如 `alice.vrm`、`bob.glb`）。

放置步骤：

1. 找到你的 Minecraft 游戏根目录（`.minecraft` 文件夹）
2. 进入其中的 `version` 目录，找到你的游戏版本文件夹（如 `1.21.1`）
3. 在版本文件夹里创建 `vrmcreature/vrm/` 目录
4. 把你的 `.vrm` / `.glb` 模型文件放入该目录，文件名使用英文字母（如 `model.vrm`、`alice.vrm`、`knight.glb`）
5. 打开「VRM 生物设置」→「现在生成」，在确认弹窗中**勾选多个模型**（支持全选 / 清空 / 翻页），点击「确认生成」后，生成 `数量` 只生物会按所选模型**循环分配外观**。例如勾选 2 个模型、数量为 4，则每只依次使用模型 A、B、A、B

最终路径示例：`.minecraft/versions/1.21.1/vrmcreature/vrm/alice.vrm`

> **自然刷新同样生效**：模型不只在手动生成时使用，世界中**自然刷新**的生物也会从该目录下所有可用模型中**循环分配外观**，与「选择刷新群系」相互独立——群系决定在哪里刷，模型决定长什么样。
> 更换模型：直接把新 `.vrm` / `.glb` 文件放进目录即可，无需重启游戏。
...
- **生物没有模型 / 显示空白？** 模型不是内置的，请按「更换你自己的模型」把模型文件放到 `.minecraft/versions/<版本名>/vrmcreature/vrm/` 目录（至少一个模型文件）。
