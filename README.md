# VRM Creature

把你的 VRM 模型变成《我的世界》里的生物！

在 Minecraft 中加载自定义 VRM 模型并生成可互动的生物：阵营（敌方 / 中立 / 我方）、生命值、攻击力、移动速度、刷新数量全部可以在游戏内设置。

**支持版本：Minecraft 1.21.1 + NeoForge 21.1.x**

## 功能特性

- 加载任意 VRM 模型，兼容 **VRM 0.x** 与 **VRM 2.0（VRMC_vrm）** 格式
- **多模型支持**：`.minecraft/version/<版本名>/vrmcreature/vrm/` 目录下可放置多个模型文件，生成时可一次**多选**多个模型
- 自然刷新开关：可选择是否在世界中自然生成
- **群系可选**：可指定只在选中的群系自然刷新（多选，默认全部群系）
- 阵营可切换：敌方（主动攻击）/ 中立（被打才反击）/ 我方（与玩家友好）
- 属性可配置：生命值、攻击力、移动速度、刷新权重与数量
- 游戏内设置界面，无需手动改配置文件

## 安装方法

1. 安装 NeoForge 21.1.x（对应 Minecraft 1.21.1）
2. 从本仓库的 **Releases** 页面下载最新版 `vrmcreature-1.0.0.jar`
3. 把 jar 文件放进游戏目录下的 `mods` 文件夹
4. 启动游戏，模组自动生效

## 使用方法

### 第一步：绑定快捷键

本模组的设置界面**默认不绑定任何按键**（避免与其他模组冲突）。进入游戏后：

1. 打开 **选项 → 控制**
2. 找到「VRM 生物」分类
3. 为「打开 VRM 生物设置」指定一个键（例如 `V`）

### 第二步：生成生物

1. 按刚才设置的快捷键，打开「VRM 生物设置」界面
2. 选择阵营：敌方 / 中立 / 我方
3. 调整生命值、攻击力、移动速度、生成数量
4. 点击「现在生成」并确认，生物立即在当前世界生成
5. 设置会自动保存，之后的自然刷新同样按此配置生效

> 小贴士：也可以直接用命令生成：`/summon vrmcreature:vrm_mob`

### 选择刷新群系（可选）

默认情况下，生物会**在全部群系**自然刷新（主世界 / 下界 / 末地）。如果你想限定只在某些群系刷新：

1. 打开「VRM 生物设置」界面
2. 点击「选择刷新群系」
3. 在列表中**勾选**允许刷新的群系（支持全选 / 清空 / 翻页，默认全选 = 全部群系）
4. 点击「保存」即可生效

> 说明：勾选数量等于全部时，等价于「全部群系」。清空后未勾选任意群系，同样按全部处理。

## 更换你自己的模型

模型直接放在游戏目录的**版本目录**下，模组从 `.minecraft/version/<版本名>/vrmcreature/vrm/` 读取（VRM / GLB 格式）。**支持一次放置多个模型**，每个模型一个文件，文件名使用英文即可（例如 `alice.vrm`、`bob.glb`）。

放置步骤：

1. 找到你的 Minecraft 游戏根目录（`.minecraft` 文件夹）
2. 进入其中的 `version` 目录，找到你的游戏版本文件夹（如 `1.21.1`）
3. 在版本文件夹里创建 `vrmcreature/vrm/` 目录
4. 把你的 `.vrm` / `.glb` 模型文件放入该目录，文件名使用英文字母（如 `model.vrm`、`alice.vrm`、`knight.glb`）
5. 打开「VRM 生物设置」→「现在生成」，在确认弹窗中**勾选多个模型**（支持全选 / 清空 / 翻页），点击「确认生成」后，生成 `数量` 只生物会按所选模型**循环分配外观**。例如勾选 2 个模型、数量为 4，则每只依次使用模型 A、B、A、B

最终路径示例：`.minecraft/version/1.21.1/vrmcreature/vrm/alice.vrm`

> **自然刷新同样生效**：模型不只在手动生成时使用，世界中**自然刷新**的生物也会从该目录下所有可用模型中**循环分配外观**，与「选择刷新群系」相互独立——群系决定在哪里刷，模型决定长什么样。
> 更换模型：直接把新 `.vrm` / `.glb` 文件放进目录即可，无需重启游戏。

> 兼容格式：VRM 0.x、VRM 2.0（VRMC_vrm）、标准 glTF（.glb）。
> 基础色贴图会自动从模型内提取并应用，无需额外准备贴图文件。
> 每个生物会记住自己使用的模型，保存/加载后仍保持对应外观。

## 配置说明

配置文件位于游戏目录 `config/vrmcreature-common.toml`，游戏内设置界面会自动修改它：

| 配置项 | 说明 |
|---|---|
| `canSpawn` | 是否允许自然刷新（true / false） |
| `spawnBiomes` | 允许自然刷新的群系 ID 列表（空列表 = 全部群系） |
| `isFriendly` | 我方阵营（与玩家友好） |
| `isNeutral` | 中立阵营（不主动攻击，被打后反击） |
| `maxHealth` | 最大生命值 |
| `attackDamage` | 攻击伤害 |
| `movementSpeed` | 移动速度 |
| `spawnWeight` | 自然刷新权重（越大越容易刷出） |
| `spawnMinGroup` / `spawnMaxGroup` | 单次刷新数量范围 |

阵营判定优先级：`isFriendly=true` → 我方；否则 `isNeutral=true` → 中立；两者都关闭 → 敌方（主动攻击玩家）。

## 常见问题

- **生物没有模型 / 显示空白？** 模型不是内置的，请按「更换你自己的模型」把模型文件放到 `.minecraft/version/<版本名>/vrmcreature/vrm/` 目录（至少一个模型文件）。
- **按键没反应？** 设置界面默认未绑定按键，请到 **选项 → 控制 → VRM 生物** 里手动指定。
- **支持哪些模型？** VRM 0.x、VRM 2.0 以及 glTF（.glb），贴图自动提取。
- **服务器能用吗？** 模组同时提供服务端与客户端支持，生成逻辑运行在服务端。

## 开源协议

MIT License
